            ...
            if not hitsWall:
                ...
                for i, corner in enumerate(self.corners):
                    if (nextx, nexty) == corner:
                        corner_flags_list = list(corner_flags)
                        corner_flags_list[i] = True
                        corner_flags = tuple(corner_flags_list)

                successors.append((((nextx, nexty), corner_flags), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    if corner_flags[0] and corner_flags[1] and corner_flags[2] and corner_flags[3]:
        return 0
...
def foodHeuristic(state, problem):
    ...
    heuristic = min([util.manhattanDistance(position, point) for point in remaining_food])

    heuristic2 = 0

    for food1 in remaining_food:
        for food2 in remaining_food:
            heuristic2 = max(heuristic2, util.manhattanDistance(food1, food2))
